...
Router.get("/search_attribute", (req, res) => {
  const searchTerm = req.query.searchTerm || "";
  let query = "SELECT * FROM attribute_table";
  ...
  if (searchTerm) {
    switch (req.query.filter) {
      ...
      case "value":
        ...
        break;
      case "timestamp":
        query += ` WHERE DATE_FORMAT(timestamp, '%d/%m/%Y %H:%i:%s') LIKE '%${searchTerm}%'`;
        break;
      default:
        query += ` WHERE attribute LIKE '%${searchTerm}%' OR value LIKE '%${searchTerm}%' OR DATE_FORMAT(timestamp, '%d/%m/%Y %H:%i:%s') LIKE '%${searchTerm}%'`;
        break;
    }
  }
...
Router.get("/alert_count", (req, res) => {
  const query = `
    ...
    WHERE device = 'canhbao'
      AND status = 'On'
      AND DATE(measurementTime) = CURDATE()
  `;
